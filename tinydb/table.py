...
class Document(dict):
    """A document stored in the database with content and a unique ID."""
    def __init__(self, value: Mapping, doc_id: int):
        super().__init__(value)
        self.doc_id = doc_id

class Table:
    """Represents a TinyDB table with methods for data manipulation and queries."""
    ...
    def __repr__(self):
        total_docs = len(self)
        return f"<{type(self).__name__} name={self._name!r}, total={total_docs}, storage={self._storage}>"
    ...
    def insert(self, document: Mapping) -> int:
        self._ensure_is_mapping(document)
        doc_id = self._resolve_document_id(document)
        self._perform_insert(document, doc_id)
        return doc_id
    ...
    def search(self, cond: QueryLike) -> List[Document]:
        return self._search_with_cache(cond)

    def get(self, cond: Optional[QueryLike] = None, doc_id: Optional[int] = None, doc_ids: Optional[List[int]] = None) -> Optional[Union[Document, List[Document]]]:
        if doc_id is not None:
            return self._get_by_id(doc_id)
        elif doc_ids is not None:
            return self._get_by_ids(doc_ids)
        elif cond is not None:
            return self._get_by_condition(cond)
        else:
            raise RuntimeError('Specify a condition or document ID(s)')

    def contains(self, cond: Optional[QueryLike] = None, doc_id: Optional[int] = None) -> bool:
        return self.get(cond=cond, doc_id=doc_id) is not None
    ...
    def update(self, fields: Union[Mapping, Callable[[Mapping], None]], cond: Optional[QueryLike] = None, doc_ids: Optional[Iterable[int]] = None) -> List[int]:
        return self._update_documents(fields, cond, doc_ids)

    def remove(self, cond: Optional[QueryLike] = None, doc_ids: Optional[Iterable[int]] = None) -> List[int]:
        return self._remove_documents(cond, doc_ids)
    ...
    def __iter__(self) -> Iterator[Document]:
        return (self.document_class(doc, self.document_id_class(doc_id)) for doc_id, doc in self._read_table().items())

    def _ensure_is_mapping(self, document):
        if not isinstance(document, Mapping):
            raise ValueError('Document must be a Mapping')

    def _resolve_document_id(self, document):
        if isinstance(document, Document):
            return document.doc_id
        return self._get_next_id()

    def _perform_insert(self, document, doc_id):
        def insert_logic(table: dict):
            if doc_id in table:
                raise ValueError(f'Document ID {doc_id} already exists')
            table[doc_id] = dict(document)
        self._update_table(insert_logic)

    def _search_with_cache(self, cond: QueryLike) -> List[Document]:
        cached_results = self._query_cache.get(cond)
        if cached_results is not None:
            return cached_results[:]
        results = self._execute_search(cond)
        self._update_query_cache(cond, results)
        return results

    def _execute_search(self, cond):
        return [self.document_class(doc, self.document_id_class(doc_id)) for doc_id, doc in self._read_table().items() if cond(doc)]

    def _update_query_cache(self, cond, results):
        if self._is_query_cacheable(cond):
            self._query_cache[cond] = results[:]

    def _is_query_cacheable(self, cond):
        is_cacheable = getattr(cond, 'is_cacheable', lambda: True)
        return is_cacheable()

    def _get_by_id(self, doc_id: int) -> Optional[Document]:
        doc = self._read_table().get(str(doc_id))
        return self.document_class(doc, doc_id) if doc else None

    def _get_by_ids(self, doc_ids: List[int]) -> List[Document]:
        ids_set = set(map(str, doc_ids))
        return [self.document_class(doc, int(doc_id)) for doc_id, doc in self._read_table().items() if doc_id in ids_set]

    def _get_by_condition(self, cond: QueryLike) -> Optional[Document]:
        for doc_id, doc in self._read_table().items():
            if cond(doc):
                return self.document_class(doc, int(doc_id))
        return None

    def _update_documents(self, fields, cond, doc_ids) -> List[int]:
        updater = self._create_document_updater(fields, cond, doc_ids)
        return self._update_table(updater)

    def _create_document_updater(self, fields, cond, doc_ids):
        def updater(table: dict):
            updated_ids = []
            for doc_id, doc in table.items():
                if (cond and cond(doc)) or (doc_ids and doc_id in doc_ids):
                    self._apply_update(doc, fields)
                    updated_ids.append(doc_id)
            return updated_ids
        return updater

    def _apply_update(self, doc, fields):
        if callable(fields):
            fields(doc)
        else:
            doc.update(fields)

    def _remove_documents(self, cond, doc_ids) -> List[int]:
        updater = self._create_remove_updater(cond, doc_ids)
        return self._update_table(updater)

    def _create_remove_updater(self, cond, doc_ids):
        def updater(table: dict):
            removed_ids = []
            for doc_id in list(table.keys()):
                if (cond and cond(table[doc_id])) or (doc_ids and doc_id in doc_ids):
                    removed_ids.append(doc_id)
                    del table[doc_id]
            return removed_ids
        return updater
    ...
    def _get_next_id(self):
        if self._next_id is None:
            existing_ids = [int(k) for k in self._read_table().keys()]
    ...
    def _update_table(self, update_logic: Callable[[Dict[int, Mapping]], None]) -> List[int]:
        tables = self._storage.read() or {}
        table = {int(k): v for k, v in tables.get(self.name, {}).items()}
        updated_ids = update_logic(table)
        tables[self.name] = {str(k): v for k, v in table.items()}
        self._storage.write(tables)
        self.clear_cache()
        return updated_ids
